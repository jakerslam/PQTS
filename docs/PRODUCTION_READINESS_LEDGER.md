...
| Production data plane | `PGQ-9`, `PGQ-14` | Started | `src/adapters/prediction_market_*`, `src/research/prediction_market_microstructure.py`, `src/research/prediction_market_capture.py`, `src/research/prediction_market_replay.py`, `scripts/capture_prediction_market_snapshots.py`, `scripts/build_prediction_market_replay.py` | Need entitlement manifests, real feed captures, and order/trade/resolution joins. | Raw order book/trade/resolution data can replay features point-in-time. |
...
- [x] Prediction-market raw snapshot capture has a CLI and append-only manifest contract.
