...
class StrategySearchSpace:
    ...
    def __init__(self):
        self.features = {
            'order_book': ['ob_imbalance', 'ob_depth_ratio', 'ob_spread', 'ob_pressure', 'cancel_rate'],
            'trade_flow': ['flow_aggressor_ratio', 'large_trade_flow', 'volume_velocity', 'trade_imbalance'],
            'volatility': ['realized_vol_1h', 'realized_vol_24h', 'vol_regime', 'volatility_percentile', 'atr_ratio'],
            'momentum': ['price_momentum_5m', 'price_momentum_1h', 'rsi_14', 'macd_signal'],
            'microstructure': ['kyle_lambda', 'amihud_illiquidity', 'price_impact', 'quote_intensity']
        }
        self.parameter_spaces = {
            'market_making': {'spread_bps': [20, 30, 50, 75, 100], 'skew_factor': [0.0, 0.25, 0.5, 0.75, 1.0]},
            'stat_arb': {'entry_zscore': [1.5, 2.0, 2.5, 3.0], 'exit_zscore': [0.3, 0.5, 0.7]},
            'trend_following': {'lookback_periods': [10, 20, 50], 'entry_threshold': [0.5, 1.0, 1.5]}
        }
...
class AutoStrategyGenerator:
    ...
    def generate_feature_combinations(self, strategy_type: str, n_features: int = 3) -> List[List[str]]:
    ...
    def generate_strategy_variants(self, strategy_type: str, n_per_feature_set: int = 5) -> List[StrategyVariant]:
        ...
        for features in feature_combos:
            ...
            for params in param_variants[:n_per_feature_set]:
                ...
                variant_str = f"{strategy_type}_{'_'.join(features[:2])}_{json.dumps(params, sort_keys=True)}"
